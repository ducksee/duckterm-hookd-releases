...
`11434` 与 `11435` 都是 Hookd 数据面监听，而不是管理端口。`11434` 属于
LAN Direct，继续受可信本地目标地址 gate 保护；显式启用后才出现的 `11435`
是 Public Direct 仅限 loopback 的代理边界，具有独立 route 与认证域。`20080`
才是本地 Web 管理面。

DuckTerm 通过移动操作系统校验 Public Direct 证书。证书过期、不受信或域名不匹配时，
该 Public 候选会失败，随后尝试其它直连 origin 或安全中继。产品不提供全局
“忽略 SSL”开关：自建站点应自动完成 ACME 续期，私有 PKI 应建立平台信任，而不是
关闭校验。只有“单 origin + 预置证书公钥 pin + 有界过期宽限”才可能成为未来可评估的
例外；无 pin 的 trust-all 模式不属于受支持设计。
